...
- **Cross-strategy meta-allocator** that splits capital across the bundled strategies (six today — obi, vpin, vwap_deviation, ema_crossover, ou, composite — plus any alphas added via the pluggable framework) based on rolling Sharpe. Adjacent to portfolio optimization — explicit non-goal today, revisit if a real driver appears. _Gated by **T10** (≥2 strategies running live and manual capital split is the bottleneck; itself behind T9)._
